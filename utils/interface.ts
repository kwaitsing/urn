...
export class Dbi {
    ...
    async get(collection: string, key: string, query: object | string | ObjectId, quantity: number = 1, sort: boolean = false, skip: number = 0, user_uuid?: ObjectId): Promise<WithId<object>[] | null[]> {
        let queryObject = {
            [key]: query
        }
